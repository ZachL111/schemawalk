...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-schemawalk-detail.ps1

ruby -Ilib tests/test_domain_review.rb

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-schemawalk-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-schemawalk-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-schemawalk-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
